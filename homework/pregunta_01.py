...
def pregunta_01():
    ...
    * Su código debe crear la carpeta `docs` si no existe.
    ...
    def create_visual_for_dome_of_shipment(df):
        df=df.copy()
        plt.figure()
    ...
    def create_visual_for_average_customer_rating(df):
        ...
        plt.figure()
        df = (
            df[["Mode_of_Shipment","Customer_rating"]].groupby("Mode_of_Shipment")
            .describe()
        )
    ...
    if not os.path.exists(carpeta):
        os.mkdir(carpeta)
        print(f'Capeta {carpeta} ha sido creada')
    else:
    ...
    create_visual_for_dome_of_shipment(df)
